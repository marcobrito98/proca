...
static void Proca_Boost_Body(const cGH* restrict const cctkGH, const int dir, const int face, const CCTK_REAL normal[3], const CCTK_REAL tangentA[3], const CCTK_REAL tangentB[3], const int imin[3], const int imax[3], const int n_subblock_gfs, CCTK_REAL* restrict const subblock_gfs[])
{
    ...
    cctk_ash[0],cctk_ash[1],cctk_ash[2])
  {
    ...
    E1xL = E1xL*pow(1 - pow(betaboost,2),-0.5);
    
    E1zL = E1zL*pow(1 - pow(betaboost,2),-0.5);
    
    E2xL = E2xL*pow(1 - pow(betaboost,2),-0.5);
    
    E2zL = E2zL*pow(1 - pow(betaboost,2),-0.5);
    /* Copy local copies back to grid functions */
    E1x[index] = E1xL;
    E1z[index] = E1zL;
    E2x[index] = E2xL;
  ...
  CCTK_ENDLOOP3(Proca_Boost);
}
extern "C" void Proca_Boost(CCTK_ARGUMENTS)
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
  ...
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Entering Proca_Boost_Body");
  }
  if (cctk_iteration % Proca_Boost_calc_every != Proca_Boost_calc_offset)
  {
    return;
  }
  ...
  const char* const groups[] = {
    "ProcaBase::E1i",
    "ProcaBase::E2i"};
  AssertGroupStorage(cctkGH, "Proca_Boost", 2, groups);
  ...
  LoopOverEverything(cctkGH, Proca_Boost_Body);
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Leaving Proca_Boost_Body");
  }
}
