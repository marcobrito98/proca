...
static void Proca_fac7_Body(const cGH* restrict const cctkGH, const int dir, const int face, const CCTK_REAL normal[3], const CCTK_REAL tangentA[3], const CCTK_REAL tangentB[3], const int imin[3], const int imax[3], const int n_subblock_gfs, CCTK_REAL* restrict const subblock_gfs[])
{
  ...
  CCTK_ENDLOOP3(Proca_fac7);
}
extern "C" void Proca_fac7(CCTK_ARGUMENTS)
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
  ...
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Entering Proca_fac7_Body");
  }
  if (cctk_iteration % Proca_fac7_calc_every != Proca_fac7_calc_offset)
  {
    return;
  }
  ...
  const char* const groups[] = {
    ...
    "ProcaBase::E1i",
    "ProcaBase::E2i"};
  AssertGroupStorage(cctkGH, "Proca_fac7", 6, groups);
  ...
  LoopOverEverything(cctkGH, Proca_fac7_Body);
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Leaving Proca_fac7_Body");
  }
}
