		...
		LC_LOOP3(global_quantities, i, j, k, 0, 0, 0, ni, nj, nk, ni, nj, nk) 	     
		{
			...
			else
			{
				...
				ul0 = w_lorentz[i3D] * ( -alp[i3D]
    				    + gxx[i3D] * vel[idx1] * betax[i3D] 
				    + gxy[i3D] * vel[idx1] * betay[i3D] 
				    + gxz[i3D] * vel[idx1] * betaz[i3D]
    				    + gxy[i3D] * vel[idx2] * betax[i3D] 
				    + gyy[i3D] * vel[idx2] * betay[i3D] 
				    + gyz[i3D] * vel[idx2] * betaz[i3D]
    				    + gxz[i3D] * vel[idx3] * betax[i3D] 
				    + gyz[i3D] * vel[idx3] * betay[i3D] 
				    + gzz[i3D] * vel[idx3] * betaz[i3D]
    				    );
